...
class FeatureExtractor(object):
    ...
    def get_result(self, word, pos):
        CD = self.word_vocab['<CD>']
        NNP = self.word_vocab['<NNP>']
        UNK = self.word_vocab['<UNK>']
        ROOT = self.word_vocab['<ROOT>']
        if pos == 'CD': return CD
        if pos == 'NNP': return NNP
        if word == None: return ROOT
        if word.lower() not in self.word_vocab: return UNK        
        return self.word_vocab[word.lower()]

    def get_input_representation(self, words, pos, state):
        # TODO: Write this method for Part 2
        return_array = [0,0,0,0,0,0]
        ...
        if(len(state.stack) == 0):
            ...
            return_array[0] = self.get_result(words[state.stack[-1]], pos[state.stack[-1]])
            return_array[1] = NULL  
            return_array[2] = NULL
        elif(len(state.stack) == 2):
            return_array[0] = self.get_result(words[state.stack[-1]], pos[state.stack[-1]])
            return_array[1] = self.get_result(words[state.stack[-2]], pos[state.stack[-2]])  
            return_array[2] = NULL  
        else:
            return_array[0] = self.get_result(words[state.stack[-1]], pos[state.stack[-1]])
            return_array[1] = self.get_result(words[state.stack[-2]], pos[state.stack[-2]]) 
            return_array[2] = self.get_result(words[state.stack[-3]], pos[state.stack[-3]]) 
        ...
        if(len(state.buffer) == 0):
            ...
            return_array[3] = self.get_result(words[state.buffer[-1]], pos[state.buffer[-1]])
            return_array[4] = NULL  
            return_array[5] = NULL
        elif(len(state.buffer) == 2):
            return_array[3] = self.get_result(words[state.buffer[-1]], pos[state.buffer[-1]])
            return_array[4] = self.get_result(words[state.buffer[-2]], pos[state.buffer[-2]])  
            return_array[5] = NULL  
        else:
            return_array[3] = self.get_result(words[state.buffer[-1]], pos[state.buffer[-1]])
            return_array[4] = self.get_result(words[state.buffer[-2]], pos[state.buffer[-2]]) 
            return_array[5] = self.get_result(words[state.buffer[-3]], pos[state.buffer[-3]])     
